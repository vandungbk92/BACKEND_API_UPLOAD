...
let storage = multer.diskStorage({
  destination: function(req, file, cb) {
    cb(null, CONSTANTS.IMAGE_PATH);
  },
  filename: function(req, file, cb) {
    let dotIndex = file.originalname.lastIndexOf('.');
    let fileName = file.originalname.slice(0, dotIndex);
    let fileExtension = file.originalname.slice(dotIndex, file.originalname.length);
    let timestamp = (new Date()).getTime();
    let nameTimestamp = fileName + '-' + timestamp + fileExtension;
    cb(null, nameTimestamp);
  },
});
